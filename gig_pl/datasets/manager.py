...
class GraphDatasetManager:
    ...
    def _get_loader(self, dataset, batch_size=1, shuffle=True, drop_last_val=False):
        ...
        return DataLoader(dataset,
                          ...
                          pin_memory=True,
                          drop_last=drop_last_val) # TODO: dropes not full batch

    def get_test_fold(self, outer_idx, batch_size=1, shuffle=True, provided_dataset=None):
        outer_idx = outer_idx or 0

        idxs = self.splits[outer_idx]["test"]
        if provided_dataset is not None:
            test_data = [provided_dataset[idx] for idx in idxs]
            sampler = RandomSampler(test_data) if shuffle is True else None

            test_loader = torch_geometric.data.DataLoader(test_data,
        ...
        # if add_cath:
        #     # TODO: change. Only possible for PROTEINS dataset
        #     config = create_config('proteins.yaml')
        #     dataset = ProteinsDataModule(config, seed_number=42)
        #     dataset.prepare_data()
        #     test_data_cath = GraphDatasetSubset(dataset.dataset, idxs)
        #     new_test_data=[]
        #     for i, data in enumerate(test_data):
        #         data.cath = test_data_cath[i].cath
        #         new_test_data.append(data)
        #     test_loader = torch_geometric.data.DataLoader(new_test_data,
    ...
    def get_model_selection_fold(self, outer_idx, inner_idx=None, batch_size_train=1,
                                 batch_size_val=1,  shuffle=True, drop_last_val=False):
        outer_idx = outer_idx or 0
        ...
        train_loader = self._get_loader(train_data, batch_size_train, shuffle)

        if len(val_data) == 0:
            val_loader = None
        else:
            val_loader = self._get_loader(val_data, batch_size_val, shuffle=False,
